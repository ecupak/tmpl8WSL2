...
{
	static bool workingFolderFixed = false;
	if (!workingFolderFixed)
	{
		FixWorkingFolder(); // needed for Surfaces declared at global scope
		workingFolderFixed = true;
	}
...
{
	int w, h, n;
	unsigned char* stbData = stbi_load(file, &w, &h, &n, 0);
	...
	else if (n == 3)
	{
		...
		{
			const uchar r = stbData[i * 3 + 0];
			const uchar g = stbData[i * 3 + 1];
			const uchar b = stbData[i * 3 + 2];
